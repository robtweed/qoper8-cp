...
class QOper8 {
  constructor(obj) {
    ...
    this.name = 'QOper8-cp';
    this.build = '5.0';
    this.buildDate = '26 September 2023';
    ...
    let workerModulePath = __dirname + '/QOper8Worker.mjs';
    ...
    this.stop = async function() {
      ...
      for (const [id, worker] of workers) {
        if (isAvailable.get(+id)) {
          q.log('Child Process ' + id + ' is being stopped');
          let msg = {type: 'qoper8_terminate'};
          sendMessage(msg, worker);
          await isStopped(id);
          q.log('Child Process ' + id + ' has been stopped (1)');
        }
        else {
          q.log('Waiting for Child Process ' + id + ' to become available');
          await isNowAvailable(id);
          let msg = {type: 'qoper8_terminate'};
          sendMessage(msg, worker);
          await isStopped(id);
          q.log('Child Process ' + id + ' has been stopped (2)');
        }
      }
      q.emit('stop');
      q.log('No Child Processes are running: QOper8 is no longer handling messages');
    ...
    if (this.logging) {
      console.log('========================================================');
      console.log('qoper8-cp Build ' + this.build + '; ' + this.buildDate + ' running in process ' + process.pid);
      console.log('Max Child Process Worker Pool Size: ' + poolSize);
      console.log('========================================================');
    }
